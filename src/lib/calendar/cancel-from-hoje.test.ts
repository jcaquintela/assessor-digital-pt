// Regressão end-to-end do painel "Hoje": o consultor abre o evento e carrega
// em Arquivar. Tem de fechar cá, calar os avisos internos, apagar o evento no
// Google e não voltar às prioridades do dia seguinte (nem depois de uma ronda
// de sincronização com o evento ainda vivo do lado do Google).
...
// O que o botão "Arquivar" da ficha aberta a partir de "Hoje" desencadeia.
const arquivarPeloDashboard = () => archiveFollowUpEverywhere(supabase, USER, FU);

describe("regressão: arquivar evento a partir do painel Hoje", () => {
  beforeEach(seed);

  it("fecha o seguimento, cala os avisos e apaga o evento no Google", async () => {
    await arquivarPeloDashboard();

    expect(db.follow_ups[0].archived_at).toBeTruthy();
    expect(String(db.follow_ups[0].status).toLowerCase()).toBe("cancelado");
  ...
  it("no dia seguinte não volta às prioridades nem tem avisos activos", async () => {
    await arquivarPeloDashboard();
    ...
    expect(db.reminders.filter((r) => r.status === "scheduled")).toHaveLength(0);
  });

  it("a sincronização de entrada não o ressuscita", async () => {
    await arquivarPeloDashboard();
    gatewayCalls.length = 0;
    externalItems = [{
