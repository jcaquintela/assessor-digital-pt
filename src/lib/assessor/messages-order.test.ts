...
const calls: { ascending?: boolean; limit?: number; gte?: string } = {};
const rows = [
...
describe("leitura da conversa", () => {
  ...
  it("limita a leitura à janela do dia corrente", async () => {
    await loadMessages(200);
    expect(calls.gte).toBeTruthy();
    expect(Date.now() - Date.parse(calls.gte!)).toBeLessThanOrEqual(24 * 60 * 60 * 1000);
  });
});
