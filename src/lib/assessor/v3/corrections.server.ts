...
export const CORRECTION_HINT_RE =
  /(?:^|\W)(n[ãa]o\s+[eé]|n[ãa]o\s+era|errado|erraste|queria\s+dizer|esse\s+n[ãa]o|essa\s+n[ãa]o|n[ãa]o\s+hoje|n[ãa]o\s+amanh[ãa]|mudei\s+de\s+ideias|apaga|cancela|desmarca|corrige|troca|substitui|ali[áa]s|n[ãa]o\s+quero|(?:isso|essa|esse)\s+[eé]\s+(?:uma?\s+)?(?:reuni[ãa]o\s+)?intern[oa]|n[ãa]o\s+(?:devias?|era\s+para)\s+perguntar)(?:$|\W)/i;
