...
function InviteLinkDialog({ user, onClose }: { user: AccessUser; onClose: () => void }) {
  ...
  const gerar = async (enviar: boolean) => {
    setBusy(true);
    try {
      const r = await issueFn({ data: { target_user_id: user.id, canal, enviar } });
      setRes(r);
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="admin-surface">
        <DialogHeader>
          <DialogTitle>Link de acesso — {user.name || user.email}</DialogTitle>
          <DialogDescription>
