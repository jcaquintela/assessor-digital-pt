...
export function CategoriesBar({
  ...
  async function criar() {
    ...
    setBusy(true);
    try {
      await create({ data: { name: nome.trim() } });
      setNome("");
      invalidate();
  ...
  async function guardarNome(id: string) {
    ...
    setBusy(true);
    try {
      await rename({ data: { id, name: editNome.trim() } });
      setEditId(null);
      invalidate();
          ...
          editId === c.id ? (
            <span key={c.id} className="flex items-center gap-1">
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") guardarNome(c.id);
                  if (e.key === "Escape") setEditId(null);
                }}
                className="h-8 w-40"
              />
              <button type="button" aria-label="Guardar nome" className="c-badge" onClick={() => guardarNome(c.id)}>
            ...
            <span key={c.id} className="flex items-center gap-1">
              <button
                type="button"
                className={"c-pill" + (selected === c.id ? " active" : "")}
              ...
              <button
                ...
                className="c-badge"
                onClick={() => { setEditId(c.id); setEditNome(c.name); }}
              >
...
export function FileCategoryDialog({
  ...
  useEffect(() => { if (open) setNova(""); }, [open, fileId]);
  ...
  async function criarEatribuir() {
    ...
    setBusy(true);
    try {
      const cat = await create({ data: { name: nova.trim() } });
